...
void testsizes(void)
{
	...
	if (sizeof(U16B) != 2)
		error(0, ERR_SIZE, "U16B");
	if (sizeof(S16B) != 2)
		error(0, ERR_SIZE, "S16B");
	if (sizeof(U32B) != 4)
		error(0, ERR_SIZE, "U32B");
	if (sizeof(S32B) != 4)
		error(0, ERR_SIZE, "S32B");
}
...
void cu_relax(void *mark)
{
	...
	for (ptr = cuhead.next; ptr != NULL && ptr != mark; ptr = ptr->next) {
		if (ptr->flags & CU_CANRELAX)
			ptr->flags |= CU_RELAXED;
	}
}
...
void cu_do(void *mark)
{
	...
	for (ptr = cuhead.next; ptr != NULL && ptr != mark;) {
		if ((ptr->flags & CU_FUNC) && ptr->arg.func != NULL) {
			if (!(ptr->flags & CU_RELAXED))
				ptr->arg.func();
		} else if ((ptr->flags & CU_RMFILE)
			   && ptr->arg.fileinfo.name != NULL) {
			close(ptr->arg.fileinfo.handle);
			if (!(ptr->flags & CU_RELAXED)
			    && remove(ptr->arg.fileinfo.name) < 0) {
				error(0, ERR_REMOVE, ptr->arg.fileinfo.name);
			}
			free(ptr->arg.fileinfo.name);
		} else if ((ptr->flags & CU_RMDIR)
			   && ptr->arg.fileinfo.name != NULL) {
			if (!(ptr->flags & CU_RELAXED))
				rmdir(ptr->arg.fileinfo.name);
			free(ptr->arg.fileinfo.name);
		}
...
char *getname(char *fullpath)
{
	...
	for (i = j = strlen(fullpath); --i >= 0;) {
		if ((unsigned char)fullpath[i] == 0xff) {
			if ((name = malloc(j - i)) == NULL)
				error(1, ERR_MEM, "getname()");
...
const char *getpath(char *fullpath)
{
	...
	for (i = strlen(fullpath); --i;) {
		if ((unsigned char)fullpath[i] == 0xff) {
			if ((path = malloc(i + 2)) == NULL)
				error(1, ERR_MEM, "getpath()");
...
char *fullpath(char *path, char *name)
{
	...
	if (path == NULL || *path == 0)
		return name;
	if ((unsigned char)path[strlen(path) - 1] != 0xff)
		need_delim = 1;
	else
		need_delim = 0;
	if ((fullpath =
	     malloc(strlen(path) + strlen(name) + need_delim + 1)) == NULL) {
...
void makepath(char *hapath)
{
	...
	for (last = strchr(hapath, 0xff); last != NULL;
	     last = strchr(last + 1, 0xff)) {
		*last = 0;
		if (access((path = md_tomdpath(hapath)), F_OK)) {
			if (mkdir(path, DEF_DIRATTR) < 0)
				error(0, ERR_MKDIR, path);
		}
		*last = -1;
	}
}
...
static int matchpattern(const char *matchpath, char *matchpat, char *path,
			char *name)
{

	if (((*matchpath == 0 && skipemptypath && sloppymatch) ||
	     strcmp(matchpath, path) == 0) && md_namecmp(matchpat, name))
		return 1;
	return 0;
}
...
int match(char *path, char *name)
{
	...
	for (i = 0; i < patcnt; ++i) {
		...
		if (matchpattern(getpath(fullhapath), getname(fullhapath),
				 md_strcase(path), md_strcase(name)))
			return 1;
	}
	return 0;
}
