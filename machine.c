...
static U16B md_tohaattr(mode_t mdattr)
{
	...
	if (S_ISDIR(mdattr))
		haattr |= HA_IFDIR;
	else if (S_ISCHR(mdattr))
		haattr |= HA_IFCHR;
	else if (S_ISBLK(mdattr))
		haattr |= HA_IFBLK;
#ifdef S_ISLNK
	else if (S_ISLNK(mdattr))
		haattr |= HA_IFLNK;
#endif
#ifdef S_ISFIFO
	else if (S_ISFIFO(mdattr))
		haattr |= HA_IFIFO;
#endif
#ifdef S_ISSOCK
	else if (S_ISSOCK(mdattr))
		haattr |= HA_IFSOCK;
#endif
	return haattr;
}
...
static char *attrstring(unsigned haattr)
{
	...
	sprintf(as, "%c%c%c%c%c%c%c%c%c%c",
		HA_ISFIFO(haattr) ? 'f' : HA_ISSOCK(haattr) ? 's' :
		HA_ISLNK(haattr) ? 'l' : HA_ISDIR(haattr) ? 'd' :
		HA_ISCHR(haattr) ? 'c' : HA_ISBLK(haattr) ? 'b' : '-',
		(haattr & HA_IRUSR) ? 'r' : '-',
		(haattr & HA_IWUSR) ? 'w' : '-',
		(haattr & HA_ISUID) ? (haattr & HA_IXUSR) ? 's' : 'S' : (haattr
									 &
									 HA_IXUSR)
		? 'x' : '-', (haattr & HA_IRGRP) ? 'r' : '-',
		(haattr & HA_IWGRP) ? 'w' : '-',
		(haattr & HA_ISGID) ? (haattr & HA_IXGRP) ? 's' : 'S' : (haattr
									 &
									 HA_IXGRP)
		? 'x' : '-', (haattr & HA_IROTH) ? 'r' : '-',
		(haattr & HA_IWOTH) ? 'w' : '-',
		(haattr & HA_ISVTX) ? (haattr & HA_IXOTH) ? 't' : 'T' : (haattr
									 &
									 HA_IXOTH)
		? 'x' : '-');
	return as;
...
void md_gethdr(int len, int mode)
{
	...
	if (len > longest) {
		if (buf != NULL)
		{
			tmp_buf = realloc(buf, len);
			if (tmp_buf != NULL)
				buf = tmp_buf;
		} else
			buf = malloc(len);
		if (buf == NULL)
			error(1, ERR_MEM, "md_gethdr()");
		longest = len;
	}
...
int md_filetype(char *path, char *name)
{
	...
	free(fullpath);
	if (filestat.st_ino == arcstat.st_ino)
		return T_SKIP;
...
int md_special(char *fullname, unsigned char **sdata)
{
	...
	if (HA_ISCHR(mdhd.attr) || HA_ISBLK(mdhd.attr)) {
		size_t dev_t_size = sizeof(dev_t);
		if ((dat = malloc(dev_t_size)) == NULL)
			error(1, ERR_MEM, "md_special()");
		*(dev_t *) dat = filestat.st_rdev;
		*sdata = dat;
		return sizeof(dev_t);
	}
	if (HA_ISLNK(mdhd.attr)) {
		if ((dat = malloc(1024)) == NULL)
			error(1, ERR_MEM, "md_special()");
...
int md_mkspecial(char *ofname, unsigned sdlen, unsigned char *sdata)
{
	...
	if (HA_ISCHR(mdhd.attr)) {
		mknod(ofname, md_tomdattr(mdhd.attr) | S_IFCHR,
		      *(dev_t *) sdata);
		if (useattr)
			chown(ofname, mdhd.user, mdhd.group);
		return 1;
	} else if (HA_ISBLK(mdhd.attr)) {
		mknod(ofname, md_tomdattr(mdhd.attr) | S_IFBLK,
		      *(dev_t *) sdata);
		if (useattr)
			chown(ofname, mdhd.user, mdhd.group);
		return 1;
		...
		if (symlink(ofname, (char *)sdata) < 0)
			error(0, ERR_MKLINK, sdata, ofname);
		if (useattr) {
...
char *md_timestring(unsigned long t)
{
	...
	sprintf(ts, "%04d-%02d-%02d  %02d:%02d", tim->tm_year + 1900,
...
char *md_arcname(char *name_req)
{
	...
	if (pos > 3 &&
	    ...
	    tolower(name_req[pos - 2]) == 'h' && name_req[pos - 3] == '.')
		return name_req;
	if ((newname = malloc(pos + 4)) == NULL)
		error(1, ERR_MEM, "md_arcname()");
...
char *md_tohapath(const char *mdpath)
{
	...
	if (i == 0)
		...
		skipemptypath = 0;
	if ((hapath = malloc(j + 1 - i)) == NULL)
		error(1, ERR_MEM, "md_tohapath()");
...
char *md_pconcat(int delim2, char *head, char *tail)
{
	...
	if ((newpath =
	     malloc(headlen + strlen(tail) + delim2 + delim1 + 1)) == NULL)
