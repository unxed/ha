...
static char *getstring(void)
{
	...
	if ((sptr = malloc(STRING)) == NULL)
		error(1, ERR_MEM, "getstring()");
	for (offset = 0;; offset++) {
		if (read(arcfile, sptr + offset, 1) != 1)
			error(1, ERR_READ, arcname);
		if (sptr[offset] == 0)
			break;
...
static void putstring(char *string)
{
	...
	len = strlen(string) + 1;
	if (write(arcfile, string, len) != len)
		error(1, ERR_WRITE, arcname);
}
...
static Fheader *getheader(void)
{
	...
	static Fheader hd = { 0, 0, 0, 0, 0, 0, NULL, NULL, 0, 0 };
	...
	if ((hd.ver = getvalue(1)) != 0xff) {
		...
		if (hd.ver < LOWVER)
			error(1, ERR_TOOOLD);
		if (hd.type != M_SPECIAL && hd.type != M_DIR
		    && hd.type >= M_UNK)
		...
		if (ipos == opos)
			...
			while (cpylen) {
				...
				len =
				    read(arcfile, ib,
					 BLOCKLEN >
					 cpylen ? (unsigned)cpylen : BLOCKLEN);
				if (len <= 0)
					error(1, ERR_READ, arcname);
				cpylen -= len;
				ipos += len;
...
static U32B arc_scan(void)
{
	...
	for (i = 0; i < arccnt; ++i) {
		...
		if (lseek(arcfile, pos, SEEK_SET) < 0)
...
void arc_open(char *aname, int mode)
{
	...
	if ((arcfile =
	     open(arcname, (mode & ARC_RDO) ? AO_RDOFLAGS : AO_FLAGS)) >= 0) {
		if (fstat(arcfile, &arcstat) != 0)
			error(1, ERR_STAT, arcname);
		...
		if (!quiet)
			printf("\nNew archive : %s\n", arcname);
		if (write(arcfile, "HA\000", 4) != 4)
			error(1, ERR_WRITE, arcname);
		arccnt = 0;
		arcsize = 4;
	} else
		error(1, ERR_ARCOPEN, arcname);
...
void arc_delete(void)
{
	...
	if (write(arcfile, "\xff", 1) != 1)
		error(1, ERR_WRITE, arcname);
	if (lseek(arcfile, 2, SEEK_SET) < 0)
		error(1, ERR_SEEK, "arc_delete()");
	putvalue(--arccnt, 2);
	dirty = 1;
}

void arc_newfile(const char *mdpath, char *name)
{
	...
	newhdr.mylen =
	    newhdr.mdilen + 20 + strlen(newhdr.path) + strlen(newhdr.name);
...
void arc_trynext(void)
{
	...
	if (lseek(arcfile, trypos, SEEK_SET) < 0)
		error(1, ERR_SEEK, "arc_trynext()");
	if (addtries++)
		dirty = 1;
}
...
int arc_addfile(void)
{
	...
	if ((basepos = arcsize + newhdr.mylen) != bestpos) {
		...
		while (len) {
			...
			if (read(arcfile, ib, cplen) != cplen)
				error(1, ERR_READ, arcname);
			len -= cplen;
			bestpos += cplen;
			if (lseek(arcfile, basepos, SEEK_SET) < 0)
				error(1, ERR_SEEK, "arc_addfile()");
			if (write(arcfile, ib, cplen) != cplen)
				error(1, ERR_WRITE, arcname);
			basepos += cplen;
		}
	}
