...
static void release_cfblocks(void)
{
	...
	do {
		...
		while (nb[nrel] == NIL) ;
		for (i = 0; i <= usp; ++i)
			if ((cps[i] & 0x7fff) == nrel)
				break;
	} while (i <= usp);
	for (i = nb[nrel], d = fa[nrel]; i != NIL; i = nb[i])
		if (fa[i] < d)
			d = fa[i];
	++d;
	if (fa[nrel] < d) {
...
static void add_model(S16B c)
{
	...
	while (usp != 0) {
		...
		if (cp & 0x8000) {
			...
			--fe[cp];
		if ((fa[i] << 1) < ++ft[cp] / (cc[cp] + 1))
			--rfm[cp];
		else if (rfm[cp] < RFMINI)
			++rfm[cp];
	...
	for (i = cslen - 1; i >= 0; --i) {
		...
		for (cp = ht[k]; cp != NIL; cp = hp[cp]) {
			if (cl[cp] == i) {
				switch (i) {
				case 4:
					if (curcon[3] != con[cp][3])
						break;
				case 3:
					if (curcon[2] != con[cp][2])
						break;
				case 2:
					if (curcon[1] != con[cp][1])
						break;
				case 1:
					if (curcon[0] != con[cp][0])
						break;
				case 0:
					cslen = i;
					return cp;
