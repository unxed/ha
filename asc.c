...
static void codepair(S16B l, S16B p)
{
	...
	if ((cf = ltab[LTCODES + i]) == 0) {
		...
		for (k = i <= LPLEN ? 0 : i - LPLEN;
		     k < (i + LPLEN >= LTCODES - 1 ? LTCODES - 1 : i + LPLEN);
		     ++k) {
			if (eltab[LTCODES + k])
				tupd(eltab, LTCODES, MAXLT, 1, k);
		}
	} else {
		tot = ltab[1] + les;
		for (lt = 0, k = LTCODES + i; k; k >>= 1) {
...
static void codechar(S16B c)
{
	...
	if ((cf = ctab[CTCODES + c]) == 0) {
		...
		for (i = c <= CPLEN ? 0 : c - CPLEN;
		     i < (c + CPLEN >= CTCODES - 1 ? CTCODES - 1 : c + CPLEN);
		     ++i) {
			if (ectab[CTCODES + i])
...
void asc_pack(void)
{
	...
	for (swd_findbest(); swd_char >= 0;) {
		if (swd_mlf > MINLEN
		    || (swd_mlf == MINLEN && swd_bpos < MINLENLIM)) {
			omlf = swd_mlf;
			obpos = swd_bpos;
			oc = swd_char;
			swd_findbest();
		...
		if (ttab[ttcon][0] > tv) {
			...
			if (tv >= ctab[1]) {
				...
				for (i = l < CPLEN ? 0 : l - CPLEN;
				     i < (l + CPLEN >=
					  CTCODES - 1 ? CTCODES - 1 : l +
					  CPLEN); ++i) {
					if (ectab[CTCODES + i])
						tupd(ectab, CTCODES, MAXCT, 1,
						     i);
				}
			} else {
				for (l = 2, lt = 0;;) {
			...
			if (tv >= ltab[1]) {
				...
				for (i = l < LPLEN ? 0 : l - LPLEN;
				     i < (l + LPLEN >=
					  LTCODES - 1 ? LTCODES - 1 : l +
					  LPLEN); ++i) {
					if (eltab[LTCODES + i])
						tupd(eltab, LTCODES, MAXLT, 1,
						     i);
				}
			} else {
				for (l = 2, lt = 0;;) {
