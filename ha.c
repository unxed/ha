...
static int fulllist = 0, usepath = 1, yes = 0, touch = 0, recurse = 0, savedir =
    0, move = 0;
static const char *defpat[] = { ALLFILES };
...
static void makeinfo(char *ifile, char *ofile)
{
	...
	if ((df = open(ifile, O_RDONLY | O_BINARY)) < 0 ||
	    (arcfile =
	     open(ofile, O_WRONLY | O_BINARY | O_TRUNC | O_CREAT,
		  DEF_FILEATTR)) < 0)
		exit(99);
	setinput(df, 0, "ifile");
	outspecial = infoout;
...
__attribute__ ((format(printf, 1, 2))) static int yesno(const char *format, ...)
{
...
static void do_list(void)
{
	...
	for (;;) {
		...
		printf("\n  %-15s %-11" F_32B " %-11" F_32B
		       " %3d.%d %%   %s  %s", hd->name, hd->olen, hd->clen,
		       (hd->olen == 0 ? 100 : (int)(100 * hd->clen / hd->olen)),
		       (hd->olen ==
			0 ? 0 : (int)((1000 * hd->clen / hd->olen) % 10)),
		       md_timestring(hd->time), method[hd->type].name);
		if (fulllist) {
			printf("\n %08" FX_32B "  %s", hd->crc,
			       *hd->path ==
			       0 ? "(none)" : md_tomdpath(hd->path));
			md_listdat();
		}
		tcs += hd->clen;
		tos += hd->olen;
...
static void do_extract(void)
{
	...
	do {
		if (usepath) {
			...
			ofname = md_tomdpath(hd->name);
		switch (hd->type) {
		case M_SPECIAL:
			if (!access(ofname, F_OK)) {
				if (!yesno
				    ("\nOverwrite special file %s ? (y/n/a) ",
				     ofname))
					break;
			}
			if (!quiet) {
			...
			if (sdata != NULL)
				free(sdata);
			if (touch)
				md_setft(ofname, md_systime());
			else
				md_setft(ofname, hd->time);
			if (!quiet)
				printf("DONE");
			break;
		case M_DIR:
		...
		default:
			...
			if (of < 0) {
				...
				if ((of =
				     open(ofname,
					  ...
					  O_EXCL, DEF_FILEATTR)) < 0)
					error(0, ERR_OPEN, ofname);
			}
			setinput(arcfile, 0, arcname);
			if (quiet)
				setoutput(of, CRCCALC, ofname);
			else
			...
			if (hd->olen != 0) {
				totalsize = hd->olen;
				cumark =
				    cu_add(CU_FUNC, method[hd->type].cleanup);
...
static void do_delete(void)
{
	...
	do {
		if (!quiet) {
			printf("\nDeleting %s",
			       md_tomdpath(fullpath(hd->path, hd->name)));
			fflush(stdout);
		}
		arc_delete();
	} while ((hd = arc_seek()) != NULL);
...
static int addfile(char *path, char *name)
{
	...
	if (totalsize) {
		for (i = 0;;) {
			...
			if (metqueue[++i] == M_UNK ||
			    (metqueue[i] == M_CPY && bestsize != totalsize))
				break;
			setoutput(arcfile, 0, arcname);
	...
	if (!quiet) {
		...
		printf("%s %3d.%d %%", method[best].name,
		       (bestsize ==
			0 ? 100 : (int)(bestsize * 100 / totalsize)),
		       (bestsize ==
			0 ? 0 : (int)((bestsize * 1000 / totalsize) % 10)));
	}
	fflush(stdout);
	arc_addfile();
	if (move) {
...
static int freshentest(char *path, char *name)
{
	...
	patcnt = 1;
	arc_reset();
	if ((hd = arc_seek()) == NULL || hd->time >= md_curfiletime())
		return 0;
	return 1;
}
...
static int addindir(char *path, char *pattern)
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		case T_DIR:
			...
			break;
		case T_SPECIAL:
			if (!md_namecmp(pattern, ent->d_name) || !special)
				break;
			if (addthis(path, ent->d_name))
				found |= addspecial(path, ent->d_name);
			break;
		case T_REGULAR:
			if (!md_namecmp(pattern, ent->d_name))
				break;
			if (addthis(path, ent->d_name))
				found |= addfile(path, ent->d_name);
			break;
		}
	}
	closedir(dir);
...
static void switchparse(char *s, const char *valid)
{

	int i;

	while (*s) {
		if (strchr(valid, tolower(*s)) == NULL)
			error(1, ERR_INVSW, *s);
		switch (tolower(*s)) {
		case 'q':
			quiet = 1;
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
		usage(ERR_UNKNOWN);
	if (argc == 4 && strcmp(argv[1], "MAKEINFO") == 0)
		makeinfo(argv[2], argv[3]);
